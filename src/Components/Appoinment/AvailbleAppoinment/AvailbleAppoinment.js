...
const AvailbleAppoinment = ({date}) => {
    return (
        <div>
            <h2 className='text-center text-brand'>Available Appoinments On {date.toDateString()}</h2>
            <div className='row p-5'>
                {bookingData.map(data=> <BookingCard data={data}/>)}
            </div>
        </div>
    );
};

export default AvailbleAppoinment;